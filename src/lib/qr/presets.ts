 ...
 */
function art(p: Omit<Preset, "style"> & { style: Partial<QrStyle>; artUrl?: string }): Preset {
  return {
    id: p.id,
    name: p.name,
    category: p.category,
    ...
    style: {
      ...DEFAULT_STYLE,
      imageMode: p.artUrl ? "paint" : "none",
      minVersion: p.artUrl ? 7 : 6,
      ecc: "H",
      ...p.style,
    },
  };
...
const ART_PRESETS: Preset[] = [
  ...
  // — New 2026 wave: ten pure-style pieces, each a different module language. —
  art({
    id: "art-neon-tokyo",
    name: "Neon Tokyo",
    category: "Art",
...
const SCENE_PRESETS: Preset[] = [
  ...
  // — New 2026 wave: ten photo weaves, one per lattice the renderer can speak. —
  art({
    id: "gal-duo-dusk",
    name: "Neon Dusk",
    category: "Gallery",
...
export const SAMPLE_IMAGES: { id: string; name: string; src: string }[] = [
  ...
  { id: "dunes", name: "Dunes", src: "/samples/dunes.jpg" },
  { id: "dusk", name: "Dusk", src: "/samples/dusk.jpg" },
  { id: "mist", name: "Mist", src: "/samples/mist.jpg" },
  { id: "aurora", name: "Aurora", src: "/samples/aurora.jpg" },
  { id: "silk", name: "Silk", src: "/samples/silk.jpg" },
