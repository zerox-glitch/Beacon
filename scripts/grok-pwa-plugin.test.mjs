...
test("platform chrome overwrites share-card metas and always sets og:title", { skip: !HAS_GROK_SCAFFOLD }, () => {
  const html =
...
test("published grok.me slug is still a title fallback", { skip: !HAS_GROK_SCAFFOLD }, () => {
  const out = injectGrokPwaHead("<html><head></head></html>", {
    host: "wild-race.grok.me",
  });
...
test("emits og:image for a public host and prefers a custom card", { skip: !HAS_GROK_SCAFFOLD }, () => {
...
test("placeholder og:image appends site.color when it is 6-digit hex", { skip: !HAS_GROK_SCAFFOLD }, () => {
...
test("document title entities are not double-escaped on og:title", { skip: !HAS_GROK_SCAFFOLD }, () => {
...
test("injects into documents with no head element", { skip: !HAS_GROK_SCAFFOLD }, () => {
...
test("streaming injector matches </HEAD> case-insensitively", { skip: !HAS_GROK_SCAFFOLD }, () => {
...
test("uses the app name in the injected title tag", { skip: !HAS_GROK_SCAFFOLD }, () => {
